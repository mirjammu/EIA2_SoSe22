...
(function (Endabgabe_SS22) {
    class Aubergine extends Endabgabe_SS22.Plant {
        waterLevel = 2220;
        waterPercentage;
        waterConstant = 2220;
        waterRate = 0.12;
        ...
        setSellValue() {
            this.sellValue = (Endabgabe_SS22.moneyGlobal[6]);
        }
        setSeedValue() {
            this.seedValue = (Endabgabe_SS22.moneyGlobal[1]);
        }
        draw() {
            if (this.waterPercentage > 1 || this.waterPercentage < 0 || this.fertilizerPercentage > 1 || this.fertilizerPercentage < 0 || this.plantHealth < 0) {
                ...
                if (this.plantProgress < 40) {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgAubergineSeed, this.position.x, this.position.y);
                    this.plantProgress += this.growthRate;
                }
                else if (this.plantProgress < 80) {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgAubergineStage1, this.position.x, this.position.y);
                    this.plantProgress += this.growthRate;
                }
                else if (this.plantProgress < 100) {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgAubergineStage2, this.position.x, this.position.y);
                    this.plantProgress += this.growthRate;
                }
                else {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgAubergineStage2, this.position.x, this.position.y);
    ...
    Endabgabe_SS22.Aubergine = Aubergine;
