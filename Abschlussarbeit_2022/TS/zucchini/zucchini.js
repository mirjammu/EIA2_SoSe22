...
(function (Endabgabe_SS22) {
    class Zucchini extends Endabgabe_SS22.Plant {
        waterLevel = 2500;
        waterPercentage;
        waterConstant = 2500;
        waterRate = 0.13;
        fertilizerLevel = 2255;
        fertilizerRate = 0.4;
        fertilizerPercentage;
        fertilizerConstant = 2255;
        plantHealth = 100;
        growthRate = 0.02;
        ...
        setSellValue() {
            this.sellValue = (Endabgabe_SS22.moneyGlobal[9]);
        }
        setSeedValue() {
            this.seedValue = (Endabgabe_SS22.moneyGlobal[4]);
        }
        draw() {
            if (this.waterPercentage > 1 || this.waterPercentage < 0 || this.fertilizerPercentage > 1 || this.fertilizerPercentage < 0 || this.plantHealth < 0) {
                ...
                if (this.plantProgress < 40) {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgZucchiniSeed, this.position.x, this.position.y);
                    this.plantProgress += this.growthRate;
                }
                else if (this.plantProgress < 80) {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgZucchiniStage1, this.position.x, this.position.y);
                    this.plantProgress += this.growthRate;
                }
                else if (this.plantProgress < 100) {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgZucchiniStage2, this.position.x, this.position.y);
                    this.plantProgress += this.growthRate;
                }
                else {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgZucchiniStage2, this.position.x, this.position.y);
    ...
    Endabgabe_SS22.Zucchini = Zucchini;
})(Endabgabe_SS22 || (Endabgabe_SS22 = {}));
//# sourceMappingURL=zucchini.js.map