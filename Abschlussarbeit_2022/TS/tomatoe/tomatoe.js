...
(function (Endabgabe_SS22) {
    class Tomatoe extends Endabgabe_SS22.Plant {
        waterLevel = 2000;
        waterPercentage;
        waterConstant = 2000;
        waterRate = 0.12;
        fertilizerLevel = 2500;
        fertilizerRate = 0.2;
        fertilizerPercentage;
        fertilizerConstant = 2500;
        plantHealth = 100;
        growthRate = 0.015;
        ...
        setSellValue() {
            this.sellValue = (Endabgabe_SS22.moneyGlobal[5]);
        }
        setSeedValue() {
            this.seedValue = (Endabgabe_SS22.moneyGlobal[0]);
        }
        draw() {
            if (this.waterPercentage > 1 || this.waterPercentage < 0 || this.fertilizerPercentage > 1 || this.fertilizerPercentage < 0 || this.plantHealth < 0) {
                ...
                if (this.plantProgress < 40) {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgTomatoeSeed, this.position.x, this.position.y);
                    this.plantProgress += this.growthRate;
                }
                else if (this.plantProgress < 80) {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgTomatoeStage1, this.position.x, this.position.y);
                    this.plantProgress += this.growthRate;
                }
                else if (this.plantProgress < 100) {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgTomatoeStage2, this.position.x, this.position.y);
                    this.plantProgress += this.growthRate;
                }
                else {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgTomatoeStage2, this.position.x, this.position.y);
    ...
    Endabgabe_SS22.Tomatoe = Tomatoe;
})(Endabgabe_SS22 || (Endabgabe_SS22 = {}));
//# sourceMappingURL=tomatoe.js.map