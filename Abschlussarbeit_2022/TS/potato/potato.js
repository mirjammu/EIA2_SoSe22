...
(function (Endabgabe_SS22) {
    class Potato extends Endabgabe_SS22.Plant {
        // abstract??
        waterLevel = 1900;
        waterPercentage;
        waterConstant = 1900;
        waterRate = 0.14;
        fertilizerLevel = 2350;
        fertilizerRate = 0.25;
        fertilizerPercentage;
        fertilizerConstant = 2350;
        plantHealth = 100;
        growthRate = 0.013;
        ...
        setSellValue() {
            this.sellValue = (Endabgabe_SS22.moneyGlobal[7]);
        }
        setSeedValue() {
            this.seedValue = (Endabgabe_SS22.moneyGlobal[2]);
        }
        draw() {
            if (this.waterPercentage > 1 || this.waterPercentage < 0 || this.fertilizerPercentage > 1 || this.fertilizerPercentage < 0 || this.plantHealth < 0) {
                ...
                if (this.plantProgress < 40) {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgPotatoSeed, this.position.x, this.position.y);
                    this.plantProgress += this.growthRate;
                }
                else if (this.plantProgress < 80) {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgPotatoStage1, this.position.x, this.position.y);
                    this.plantProgress += this.growthRate;
                }
                else if (this.plantProgress < 100) {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgPotatoStage2, this.position.x, this.position.y);
                    this.plantProgress += this.growthRate;
                }
                else {
                    Endabgabe_SS22.crc2.drawImage(Endabgabe_SS22.imgPotatoStage2, this.position.x, this.position.y);
